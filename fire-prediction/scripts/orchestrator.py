...
def build_cfg(config: dict, key: str, mode: str) -> dict:
    ...
    if "raw" not in paths:
        raise KeyError(f"{key}: no raw path configured for mode '{mode}'")

    if "curated" not in paths:
        raise KeyError(f"{key}: no curated path configured for mode '{mode}'")
...
def choose_firms_operational_window(config: dict, today: datetime, fire_months, logger):
    """
    Choose the FIRMS ingest window for operational mode.

    - Use a 7-day refresh if all sensors have continuous coverage since season
      start; otherwise backfill the full current season.
    - Coverage gaps are logged as warnings through structlog (not print) so
      they appear in the structured pipeline log.
    """
        ...
        for interval_start, interval_end in intervals:
            ...
            if interval_start > cursor:
                ...
                coverage_notes.append(
                    f"{sensor}: gap {cursor} → {interval_start - timedelta(days=1)}"
                )
                break
    ...
    if continuous and sensors:
        ...
        if coverage_notes:
            # Use structlog so this appears in the structured pipeline log,
            # not silently on stdout only.
            logger.warning(
                "FIRMS archive incomplete — using full current-season backfill",
...
def run_pipeline(mode: str, force_static: bool = False):
    ...
    if mode == "train":
        ...
        # FIRMS: current-season archive needed for days_since_fire and rolling
        # windows. choose_firms_operational_window handles gap detection and
        # logs warnings through structlog (not print).
        if in_fire_season:
            firms_start, firms_end, firms_window_reason = choose_firms_operational_window(
                config, today, fire_months, logger
            )
        else:
        ...
        # ERA5-Land: structural lag of ~5 days (ERA5T). The full current-season
        # range is needed for recursive FWI state and 7-day weather features.
        era5_lag_days = config.get("operational", {}).get("era5_lag_days", 7)
        era5_end_dt   = today - timedelta(days=era5_lag_days)
        if in_fire_season:
        ...
        # Sentinel-2: fetch from start of month to capture latest cloud-free composite
    ...
    if mode == "train":
        ...
        print(f"  ERA5-Land: {era5_start} → {era5_end} (ERA5T; structural lag ~5d)")
    ...
    if mode == "train" or force_static:
        ...
        results["dem"]       = run_source("DEM",        DEMSource(build_cfg(config, "dem", mode)))
        results["worldpop"]  = run_source("WorldPop",   WorldPopSource(build_cfg(config, "worldpop", mode)))
        results["osm"]       = run_source("OSM Roads",  OSMSource(build_cfg(config, "osm", mode)))
        results["landcover"] = run_source("Land Cover", LandCoverSource(build_cfg(config, "landcover", mode)))
    ...
    if mode == "train":
        ...
        results["firms"] = run_source(
            "FIRMS (NRT)",
            FIRMSSource(build_cfg(config, "firms", mode)),
            start_date=firms_start, end_date=firms_end,
        )
        results["era5"] = run_source(
            "ERA5 (ERA5T)",
            ERA5Source(build_cfg(config, "era5", mode)),
            start_date=era5_start, end_date=era5_end,
        )
        results["sentinel"] = run_source(
            "Sentinel-2",
            SentinelSource(build_cfg(config, "sentinel", mode)),
            start_date=sentinel_start, end_date=sentinel_end,
        )
