...
def add_fire_features(communes, fire, feature_date, fire_months):
    ...
    return base.merge(today[["commune_id", "fire_count_3d", "fire_count_7d", "frp_total_7d",
                             "days_since_fire", "wilaya_fire_excl_self"]], on="commune_id", how="left")
...
def add_weather_features(base, communes, weather, mapping, feature_date):
    ...
    base = base.merge(latest.drop(columns=["date", "latitude", "longitude"], errors="ignore"),
    ...
    base = base.merge(rolling_latest[["era5_cell_id", "FWI_7d", "DC_7d", "DMC_7d",
                                      "precip_mm_7d", "FWI_trend"]], on="era5_cell_id", how="left")
...
def add_baselines(base, engineered, static, sentinel, feature_date):
    ...
    if not current_sentinel.empty:
        base = base.merge(current_sentinel[["commune_id", "NDVI", "NDWI", "NBR", "date"]].rename(
            columns={"date": "sentinel_observation_date"}), on="commune_id", how="left")
    else:
    ...
    base["month_sin"], base["month_cos"] = np.sin(2 * np.pi * month / 12), np.cos(2 * np.pi * month / 12)
...
def add_urgency_and_shap(base, X, proba, preds, model_features, xai_dir, model):
    ...
    weights = urgency.get("weights", {"risk_proba_high": .50, "pop_density": .20,
                                        "road_distance": .15, "burnable_fraction": .15})
    thresholds = urgency.get("thresholds", {"CRITICAL": 70, "HIGH PRIORITY": 50, "MONITOR": 30})
    pop_cap = caps.get("pop_density_cap") or max(base["pop_density_mean"].quantile(.95), 1e-9)
    road_cap = caps.get("road_distance_cap") or max(base["road_distance_mean_km"].quantile(.95), 1e-9)
    ...
    score = 100 * (weights.get("risk_proba_high", .50) * proba[:, 2]
                   + weights.get("pop_density", .20) * pop.clip(upper=1)
                   + weights.get("road_distance", .15) * road.clip(upper=1)
                   + weights.get("burnable_fraction", .15) * fuel)
    base["urgency_score"] = np.round(score, 1)
    base["urgency_label"] = np.select(
        [score >= thresholds.get("CRITICAL", 70), score >= thresholds.get("HIGH PRIORITY", 50),
         score >= thresholds.get("MONITOR", 30)],
        ["CRITICAL", "HIGH PRIORITY", "MONITOR"], default="ROUTINE"
    )
    low_confidence_wilayas = []
    if equity_path.exists():
        low_confidence_wilayas = json.loads(equity_path.read_text()).get("low_confidence_wilayas", [])
    base["low_confidence"] = base["wilaya_name"].astype(str).isin({str(x) for x in low_confidence_wilayas})
    base["top_drivers"] = [[] for _ in range(len(base))]
    # Record the production explanation method in the website contract.
    ...
    try:
        # Build the explainer from the exact model loaded for this run.
        # This prevents an old pickled explainer from drifting away from a
        # newly retrained or retuned model.
        ...
        # Explain every modeled commune, including LOW, MODERATE, and HIGH.
...
def run(feature_date: pd.Timestamp, root: Path, config_path: Path):
    # Stage 1: choose the feature date and next-day prediction target.
    progress(f"Starting prediction for feature date {feature_date.date()} -> target {(feature_date + pd.Timedelta(days=1)).date()}")
    ...
    # Stage 2: load frozen spatial/static artifacts. These are not current
    # observations and therefore remain sourced from the training artifacts.
    ...
    communes = gpd.read_file(boundaries).rename(columns={"GID_1": "wilaya_id", "NAME_1": "wilaya_name",
    ...
    # Stage 3: load the complete operational FIRMS current-season archive.
    # The full season is needed for days_since_fire and dense rolling windows.
    ...
    progress(f"  FIRMS coverage: {firms['date'].min() if not firms.empty else 'none'} -> {firms['date'].max() if not firms.empty else 'none'} ({len(firms):,} detections)")
    # Stage 4: load operational ERA5/FWI history through the latest available
    # reanalysis date. ERA5 may be stale; its actual date is preserved below.
    ...
    progress(f"  ERA5 coverage: {weather['date'].min().date()} -> {weather['date'].max().date()} ({len(weather):,} cell-day rows)")
    # Stage 5: load the current operational Sentinel month. If unavailable,
    # use only the latest operational month; never use old training Sentinel as
    # a fake current-year observation.
    ...
    # At a month boundary, the operational directory can contain both August
    # and September. Use September for communes with a September observation;
    # use the latest earlier month only for communes missing September data.
    ...
    current_count = len(current_month_rows)
    fallback_count = len(fallback_rows)
    progress(
        f"  Sentinel selected: {current_count:,} current-month rows, "
        f"{fallback_count:,} prior-month fallback rows"
    )
    # Stage 6: assemble source data using the frozen spatial ERA5 mapping.
    ...
    X = base[features].apply(pd.to_numeric, errors="coerce").replace([np.inf, -np.inf], np.nan).fillna(0.0)
    ...
    progress(f"Predictions: LOW={(preds == 0).sum():,}, MODERATE={(preds == 1).sum():,}, HIGH={(preds == 2).sum():,}")
    # Stage 9: calculate urgency and a class-specific explanation for every
    # modeled fire-prone commune, including LOW predictions.
    ...
    weather_lag = (feature_date - weather_date).days
    sentinel_date = pd.to_datetime(sentinel["date"]).max() if not sentinel.empty else pd.NaT
    base["prediction_date"] = feature_date.strftime("%Y-%m-%d")
    base["target_date"] = (feature_date + pd.Timedelta(days=1)).strftime("%Y-%m-%d")
    base["risk_class"] = preds.astype(int)
    base["risk_label"] = [RISK_LABELS[int(x)] for x in preds]
    base["prob_low"], base["prob_moderate"], base["prob_high"] = proba[:, 0], proba[:, 1], proba[:, 2]
    base["borderline"] = (base["prob_high"] >= 0.35) & (base["prob_high"] < 0.55)
    ...
    base["weather_observation_date"] = weather_date.strftime("%Y-%m-%d")
    base["weather_staleness_days"] = weather_lag
    base["sentinel_observation_date"] = sentinel_date.strftime("%Y-%m-%d") if not pd.isna(sentinel_date) else None
    max_staleness = cfg.get("operational", {}).get("max_weather_staleness_days", 7)
    base["data_quality_status"] = np.where(
        weather_lag >= max_staleness,
        "DEGRADED_WEATHER_FRESHNESS",
        "OK",
    )
    serving.mkdir(parents=True, exist_ok=True)
    cols = ["commune_id", "commune_name", "wilaya_id", "wilaya_name", "prediction_date", "target_date",
            "risk_class", "risk_label", "prob_low", "prob_moderate", "prob_high", "urgency_score", "urgency_label",
            "low_confidence", "borderline", "top_drivers", "xai_method", "fire_count", "mean_frp", "temp_c", "rh", "FWI", "DC", "DMC", "NDVI", "NBR", "pop_density_mean",
            "road_distance_mean_km", "forest_fraction", "burnable_fraction", "weather_observation_date",
            "weather_staleness_days", "sentinel_observation_date", "model_version", "feature_schema_version",
            "data_quality_status"]
    ...
    logger.info("ERA5 observation: {} ({} days stale)", weather_date.date(), weather_lag)
